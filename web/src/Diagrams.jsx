...
const T = ({ x, y, children, anchor = 'start', size = 13, fill = ink2, weight = 500 }) => <text x={x} y={y} fontSize={size} fill={fill} textAnchor={anchor} fontFamily="Geist, sans-serif" fontWeight={weight}>{children}</text>

export function BedBuildUp() {
  return (
    <figure className="diagram">
      <svg viewBox="0 0 720 260" role="img" aria-label="Patio build-up: 22 mm slab on a 30 to 40 mm full mortar bed on 100 mm compacted Type 1, over the sub-grade">
        ...
        <T x={500} y={86} fill={ink}>Full wet bed 30–40 mm</T><T x={500} y={101} size={12.5}>4:1 sharp sand : cement,</T><T x={500} y={116} size={12.5}>slab back primed</T>
        <T x={500} y={140} fill={ink}>MOT Type 1, 100 mm</T><T x={500} y={155} size={12.5}>two layers, each compacted</T>
        ...
        <T x={260} y={26} anchor="middle" size={12.5}>fall 1 : 80 — 12.5 mm per metre, away from the house</T>
      </svg>
  ...
  return (
    <figure className="diagram">
      <svg viewBox="0 0 640 220" role="img" aria-label="Joint widths: 10 mm for riven sandstone, 3 to 5 mm for rectified porcelain">
        ...
        <T x={355} y={80} size={12.5}>Riven edges need the room. Point with</T><T x={355} y={96} size={12.5}>3:1 sand and cement or a brush-in compound.</T>
        <T x={355} y={144} fill={ink} weight={600}>Rectified porcelain · 3–5 mm joints</T>
        <T x={355} y={162} size={12.5}>Two-part resin or a porcelain brush-in</T><T x={355} y={178} size={12.5}>compound. Plain mortar cracks against a</T><T x={355} y={194} size={12.5}>slab that doesn’t move.</T>
        <path d="M128 30 L128 40 M140 30 L140 40" stroke={sand} strokeWidth="2" /><T x={134} y={26} anchor="middle" size={12} fill={sand} weight={600}>10</T>
        <path d="M132 112 L132 122 M136 112 L136 122" stroke={sand} strokeWidth="2" /><T x={134} y={108} anchor="middle" size={12} fill={sand} weight={600}>3–5</T>
      </svg>
...
export function MixedPack() {
  ...
  return (
    <figure className="diagram">
      <svg viewBox="0 0 640 222" role="img" aria-label="The four sizes in a mixed patio pack, drawn to scale: 900 by 600, 600 by 600, 600 by 295 and 295 by 295 millimetres">
        {sizes.map(([w, h], i) => { const el = (<g key={i}><rect x={x} y={30} width={w * mm} height={h * mm} fill="#C7B9A0" stroke={ink} /><T x={x + (w * mm) / 2} y={30 + h * mm + 18} anchor="middle" size={12.5} fill={ink}>{w} × {h}</T></g>); x += w * mm + 18; return el })}
        <T x={30} y={190} size={12.5}>18.19 m² per pack · sizes are modular: two 295s plus a 10 mm joint equal a 600.</T><T x={30} y={206} size={12.5}>Laid random, no joint longer than about 1.5 m.</T>
      </svg>
  ...
  return (
    <figure className="diagram">
      <svg viewBox="0 0 640 200" role="img" aria-label="Worked example: 6 by 4 metres is 24 square metres, plus 10 percent is 26.4, which rounds up to two 18.19 square metre packs">
        ...
        <T x={120} y={95} anchor="middle" size={14} fill="#161616" weight={600}>6 m × 4 m</T>
        <T x={120} y={113} anchor="middle" size={12} fill="#3a3a3a">= 24.00 m²</T>
        ...
        <T x={250} y={164} size={12.5}>The 10 m² left over is your cuts, breakages</T><T x={250} y={180} size={12.5}>and the day a slab cracks in five years.</T>
      </svg>
