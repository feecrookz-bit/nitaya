...
| **Coleisha** | The point of contact for upgrades and changes after go-live. Copy, photographs, prices when they change, the look, the pack: all arranged through her. Works from the yard, so she can check facts across the counter. |
| **Nitya Stones** | Owns everything on the site once the build is paid for: the code, the design, the photographs as graded, the documents. Provides the answers and assets in the next section, and a WordPress/WooCommerce administrator to issue keys. |
...
- **Cloudflare**, in the company's own account (Coleisha's company email), with Fee added as a member. Done on 15 September: the site runs there as a Worker serving the static build at https://nitya-stones.coleisha.workers.dev, deployed from GitHub on every push, with the redirect rules for the old addresses; the stock system runs beside it at https://nitya-stock-api.coleisha.workers.dev. Custom domain and HTTPS follow when the DNS moves. Free for commercial use; Vercel would work but its free tier forbids it.
